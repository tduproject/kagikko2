...
def post_list(request):
    posts = PostingSubject.objects.all()
...
def index(request,pk):
    """表示・投稿を処理する"""
    posts = get_object_or_404(PostingSubject, pk=pk)
    # 教科名と投稿名者をフォームにあらかじめ登録しておく設定
    form = PostingForm(initial={'subject':posts.subject})
...
def create_thread(request):
    # ModelFormもFormもインスタンスを作るタイミングでの使い方は同じ
    form = PostingSubjectForm(request.POST)
    """表示・投稿を処理する"""
    if request.method == 'POST':
        if form.is_valid():
            ...
            return redirect('keijiban:index',pk=post.pk)
        else:
            # メッセージフレームワークを使い、処理が失敗したことをユーザーに通知する
            messages.error(request, '入力内容に誤りがあります。')
